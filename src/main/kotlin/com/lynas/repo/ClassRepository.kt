...
interface ClassRepository : GraphRepository<Course> {
    ...
    @Query("""match (p:Person) -[:studentIsAPerson]- (s:Student) -[e:Enrolment{year:{2}}]- (c:Class),
            (c)-[:classBelongsToAnOrganization]->(org:Organization) where ID(c) = {0} and ID(org) = {1}
            return ID(s) as studentId, (p.firstName +' '+ p.lastName) as studentName, e.roleNumber as roleNumber,
            ID(e) as enrolmentId Order By e.roleNumber""")
    ...
    @Query("""match (p:Person) -[:studentIsAPerson]- (s:Student) -[e:Enrolment {year: {1}}]- (c:Class)
            -[:classBelongsToAnOrganization]->(org:Organization) where ID(c) = {0} and ID(org) = {2}
            return ID(s) as studentId, (p.firstName +' '+ p.lastName) as studentName, e.roleNumber as roleNumber
            Order By e.roleNumber""")
    fun findStudentsByClass(classId: Long?, year: Int?, orgId: Long): Collection<ClassDetailQueryResult>

    @Query("""match (cl:Class)-[:classBelongsToAnOrganization]->(org:Organization) where ID(org) = {0}
            return count(cl)""")
    fun findListCountByOrganizationName(orgId: Long): Int

    @Query("""MATCH (n:Class{name:{0},shift:{1},section:{2}})-[:classBelongsToAnOrganization]->(org:Organization)
            where ID(org)={3} RETURN n LIMIT 1""")
