...
import com.lynas.dto.ExamListQueryResult
import com.lynas.dto.ExamQueryResult
import com.lynas.dto.ResultUpdateQueryResult
...
interface ExamRepository : GraphRepository<Exam> {
 ...
 @Query("""match (s:Student) -[ e:Enrolment {year: {2}} ]- (cc:Class),
            ...
            (cc) <- [c:curriculum]- (sb:SubjectResponseDTO) <- [ex:examOfSubject] -(exam:Exam{year: {2}}),
            (exam) -[info:examInfoOfPerson]-> (s) -[ss:studentIsAPerson]- (p:Person)" +
           where ID(cc) = {0} and ID(sb) = {1} and ID(org) = {3} return collect(exam) as exam, sb.subjectName as
           subject, p.firstName as person, e.roleNumber as roleNumber, cc.name as courseName""")
 ...
 @Query("""match (s:Student) -[ e:Enrolment {year: {2}} ]- (cc:Class),
            ...
            (cc) <- [c:curriculum]- (sb:SubjectResponseDTO) <- [ex:examOfSubject] -(exam:Exam{year: {2}}),
            (exam) -[info:examInfoOfPerson]-> (s) -[ss:studentIsAPerson]- (p:Person)
            where ID(cc) = {0} and ID(s) = {1} and ID(org) = {3} return collect(exam) as exam, sb.subjectName as
            subject, p.firstName as person, e.roleNumber as roleNumber, cc.name as courseName""")
 ...
 @Query("""match (s:Student) -[ e:Enrolment {year: {1}} ]- (cc:Class),
            (cc)-[:classBelongsToAnOrganization]->(org:Organization),
            (cc) <- [c:curriculum]- (sb:SubjectResponseDTO) <- [ex:examOfSubject] -(exam:Exam{year: {1}}),
            (exam) -[info:examInfoOfPerson]-> (s) -[ss:studentIsAPerson]- (p:Person)
            where ID(cc) = {0} and ID(org) = {2} return collect(exam) as exam, sb.subjectName as subject,
            p.firstName as person, e.roleNumber as roleNumber, cc.name as courseName, ID(s) as studentId""")
 ...
 @Query(""" MATCH (cc)-[:classBelongsToAnOrganization]->(org:Organization),
             (cc:Class) <- [c:curriculum]- (sb:SubjectResponseDTO) <- [ex:examOfSubject] -(exam:Exam{year: {2}})
 ...
 @Query(""" MATCH (cc)-[:classBelongsToAnOrganization]->(org:Organization),
            (cc:Class) <- [c:curriculum]- (sb:SubjectResponseDTO) <- [ex:examOfSubject] -(exam:Exam{year: {2}})
            where ID(cc) = {0} and ID(sb) = {1} and ID(org) = {3} return  distinct exam.date as date, exam.examType
            as examType """)
    fun examListBySubject(classId: Long, subjectId: Long, year: Int, orgId: Long) : List<ExamListQueryResult>

 @Query("""match (s:Student) -[ e:Enrolment {year: {2}} ]- (cc:Class),
          (cc)-[:classBelongsToAnOrganization]->(org:Organization), (cc) <- [c:curriculum]- (sb:SubjectResponseDTO)
          <- [ex:examOfSubject] -(exam:Exam{year: {2}, examType: {4}}), (exam) -[info:examInfoOfPerson]-> (s)
