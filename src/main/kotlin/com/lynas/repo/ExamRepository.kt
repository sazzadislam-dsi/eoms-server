...
interface ExamRepository : GraphRepository<Exam> {
 ...
 @Query("""match (s:Student) -[ e:Enrolment {year: {2}} ]- (cc:Class),
          ...
          (exam) -[info:examInfoOfPerson]-> (s) -[ss:studentIsAPerson]- (p:Person) where ID(cc) = {0}
          and ID(sb) = {1} and ID(org) = {3} return collect(exam) as exam, sb.subjectName as subject,
          p.firstName as person, e.roleNumber as roleNumber, cc.name as courseName, info,  s,
          exam.totalNumber as totalNumber, exam.percentile as percentile""")
